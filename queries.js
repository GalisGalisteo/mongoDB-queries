...
db.restaurants.find().forEach(function (doc) { printjson(doc) });
...
db.restaurants.find({ $and: [{ cuisine: { $not: { $eq: "American" } } }, { "grades.score": { $gt: 70 } }, { "address.coord": { $lt: -65.754168 } }] });
...
db.restaurants.find({ cuisine: { $not: { $eq: "American" } }, "grades.score": { $gt: 70 }, "address.coord": { $lt: -65.754168 } });
